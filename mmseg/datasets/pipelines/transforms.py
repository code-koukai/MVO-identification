...
@PIPELINES.register_module()
class RandomFlip_clips_horiziontal(object):
    """Flip the image & seg.
...
@PIPELINES.register_module()
class RandomFlip_clips_vertical(object):
    """Flip the image & seg.
    ...
    @deprecated_api_warning({'flip_ratio': 'prob'}, cls_name='RandomFlip')
    def __init__(self, prob=None, direction='vertical'):
        self.prob = prob
        self.direction = direction
        if prob is not None:
            assert prob >= 0 and prob <= 1
...
@PIPELINES.register_module()
class RandomFlip_clips_both_horiziontal_and_vertical(object):
    """Flip the image & seg.
            ...
            for key in results.get('seg_fields', []):
                ...
                for gt in results[key]:
                    gt_flip = mmcv.imflip(
                        gt, direction=results['flip_direction']).copy()
                    gt_flip = mmcv.imflip(
                        gt_flip, direction='vertical').copy()
                    gt_seg_all.append(gt_flip)
                results[key]=gt_seg_all
        return results
...
@PIPELINES.register_module()
class ReshuffleImages_clips(object):
    def __init__(self, loop=False):
        self.loop = loop

    def reshuffle(self, img_list, num_images, start_idx):
        """执行重排序的函数"""
        new_order = list(range(start_idx, num_images)) + list(range(0, start_idx))
        return [img_list[i] for i in new_order]
    ...
    def __call__(self, results):
        ...
        if self.loop:
            ...
            if random.randint(2):
                ...
                start_idx = random.choice([random.randint(0, 4), random.randint(24, 29)])
            else:
                start_idx = 0
        results['img'] = self.reshuffle(results['img'], num_images, start_idx)
        
        return results
    
@PIPELINES.register_module()
class NoiseAndBlur_clips(object):
    ...
    def apply_blur(self, img):
        ...
        if random.randint(2):
            kernel_size = random.choice([3, 5, 7])  # You can adjust these values
    ...
    def __call__(self, results):
        ...
        for im in results['img']:
            ...
            # mode == 0 --> add noise
            # mode == 1 --> apply blur
            # mode == 2 --> add noise and apply blur
            mode = random.randint(2)
            if mode == 0:
                img = self.add_noise(im)

            # random saturation
            elif mode == 1:
                img = self.apply_blur(im)
...
@PIPELINES.register_module()
class ElasticTransformation_clips(object):
    ...
    def elastic_transform(self,image, alpha, sigma, random_state=None):
        ...
        indices = np.vstack((y.ravel() + dy.ravel(), x.ravel() + dx.ravel()))
    ...
    def __call__(self, results):
        ...
        img_all = []
        for im in results['img']:
            img = self.elastic_transform(im, 3, 2)
            img_all.append(img)
...
@PIPELINES.register_module()
class RandomRotate_clips(object):
    """Rotate the image & seg.
