...
class SegFormerHead_clips2_resize_1_8_hypercorrelation2_topk_ensemble4(BaseDecodeHead_clips):
    ...
    def __init__(self, feature_strides, **kwargs):
        ...
        self.deco5=small_decoder2(embedding_dim,256, self.num_classes)
        self.deco6=small_decoder2(embedding_dim,256, self.num_classes)
        self.deco7=small_decoder2(embedding_dim,256, self.num_classes)
        self.deco8=small_decoder2(embedding_dim,256, self.num_classes)
        self.deco9=small_decoder2(embedding_dim,256, self.num_classes)
        self.deco10=small_decoder2(embedding_dim,256, self.num_classes)
    ...
    def forward(self, inputs, batch_size=None, num_clips=None):
        ...
        # print("input shapes", [len(item) for item in inputs])
        x = self._transform_inputs(inputs)  # len=4, 1/4,1/8,1/16,1/32
        # print("x_size", x[0].shape, x[1].shape, x[2].shape, x[3].shape)
        c1, c2, c3, c4 = x 
        ############## MLP decoder on C1-C4 ###########
        n, _, h, w = c4.shape
        ...
        # x = x.reshape(batch_size, 1, -1, h, w)
        ...
        # out5=resize(self.deco5(outs[4]), size=(h, w),mode='bilinear',align_corners=False).unsqueeze(1)
        # out6=resize(self.deco6(outs[5]), size=(h, w),mode='bilinear',align_corners=False).unsqueeze(1)
        # out7=resize(self.deco7(outs[6]), size=(h, w),mode='bilinear',align_corners=False).unsqueeze(1)
        # out8=resize(self.deco8(outs[7]), size=(h, w),mode='bilinear',align_corners=False).unsqueeze(1)
        # out9=resize(self.deco9(outs[8]), size=(h, w),mode='bilinear',align_corners=False).unsqueeze(1)
        ...
        # out10=resize(self.deco10((outs[0]+outs[1]+outs[2]+outs[3]+outs[4]+outs[5]+outs[6]+outs[7]+outs[8])/9.0+outs[9]), size=(h, w),mode='bilinear',align_corners=False).unsqueeze(1)
        
        # test clip x
        x = x[:, 3:4, :, :, :]
        # print("x shape", x.shape)
        # print("out10 shape", out10.shape)
        output4 = torch.cat([x,out4],dim=1)   ## b*(k+k)*124*h*w

        # print("output10 shape", output10.shape)
        '''torch.Size([2, 1, 4, 60, 60])
        output.shape torch.Size([2, 20, 4, 60, 60])
        seg_logits.shape torch.Size([2, 4, 60, 60])
        gt_semantic_seg.shape torch.Size([2, 10, 1, 240, 240])'''
        #output=torch.cat([x,out1,out2,out3,out4,out5,out6,out7,out8,out9,out10],dim=1)   ## b*(k+k)*124*h*w
...
@HEADS.register_module()
class SegFormerHead_clips2_resize_1_8_hypercorrelation2_topk_ensemble30(BaseDecodeHead_clips):
    """
    SegFormer: Simple and Efficient Design for Semantic Segmentation with Transformers
    use hypercorrection in hsnet
    """
    def __init__(self, feature_strides, **kwargs):
        super(SegFormerHead_clips2_resize_1_8_hypercorrelation2_topk_ensemble30, self).__init__(input_transform='multiple_select', **kwargs)
        assert len(feature_strides) == len(self.in_channels)
        assert min(feature_strides) == feature_strides[0]
        self.feature_strides = feature_strides
    ...
    def forward(self, inputs, batch_size=None, num_clips=None):
        ...
        # out4=resize(self.deco4(outs[3]+outs[2]+outs[1]+outs[0]), size=(h, w),mode='bilinear',align_corners=False).unsqueeze(1)
        # out4=resize(self.deco4((outs[3]+outs[2]+outs[1])/3.0+outs[0]), size=(h, w),mode='bilinear',align_corners=False).unsqueeze(1)
        out30 = resize(self.deco10(sum(outs[:29]) / 29.0 + outs[29]), size=(h, w), mode='bilinear', align_corners=False).unsqueeze(1)

        # test clip x
        x = x[:, 29:30, :, :, :]
        # print("x shape", x.shape)
        # print("out10 shape", out10.shape)
        output30 = torch.cat([x,out30],dim=1)   ## b*(k+k)*124*h*w
...
@HEADS.register_module()
class SegFormerHead_clips2_resize_1_8_hypercorrelation2_topk_ensemble1(BaseDecodeHead_clips):
    """
    SegFormer: Simple and Efficient Design for Semantic Segmentation with Transformers
    use hypercorrection in hsnet
    """
    def __init__(self, feature_strides, **kwargs):
        super(SegFormerHead_clips2_resize_1_8_hypercorrelation2_topk_ensemble1, self).__init__(input_transform='multiple_select', **kwargs)
        assert len(feature_strides) == len(self.in_channels)
        assert min(feature_strides) == feature_strides[0]
        self.feature_strides = feature_strides
             c1_in_channels, c2_in_channels, c3_in_channels, c4_in_channels = self.in_channels
    ...
    def forward(self, inputs, batch_size=None, num_clips=None):
        ...
        # out4=resize(self.deco4(outs[3]+outs[2]+outs[1]+outs[0]), size=(h, w),mode='bilinear',align_corners=False).unsqueeze(1)
        # # out4=resize(self.deco4((outs[3]+outs[2]+outs[1])/3.0+outs[0]), size=(h, w),mode='bilinear',align_corners=False).unsqueeze(1)
        # out1 = resize(self.deco10(sum(outs[:29]) / 29.0 + outs[29]), size=(h, w), mode='bilinear', align_corners=False).unsqueeze(1)

        # test clip x
        x = x[:, 0:1, :, :, :]
        # print("x shape", x.shape)
        # print("out10 shape", out10.shape)
        output1 = torch.cat([x,out1],dim=1)   ## b*(k+k)*124*h*w
